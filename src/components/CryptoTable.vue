<template>
  <table class="w-full divide-y divide-gray-300 table-auto">
    <thead class="">
      <tr class="">
        <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 cursor-pointer sm:pl-6 md:pl-0 truncate whitespace-nowrap max-w-[50px]" @click="sortBy('market_cap')">Rank #</th>
        <th scope="col" class="py-3.5 px-3 text-left text-sm font-semibold text-gray-900 cursor-pointer truncate whitespace-nowrap" @click="sortBy('name')">Name</th>
    ...
    <tbody class="divide-y divide-gray-200">
      <template v-if="filteredCoins.length">
        <tr v-for="coin in filteredCoins" :key="coin.id">
          ...
          <td class="px-3 py-4 text-sm text-gray-500 truncate whitespace-nowrap">{{ coin.name }} ({{ coin.symbol }})</td>
