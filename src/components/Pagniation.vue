<template>
  <div v-if="coins.length >= cryptoStore.sort.selectedLimit" class="mt-4">
      ...
      <nav class="relative z-0 inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
        ...
        <div class="flex items-center" v-for="(number, index) in pages" :key="index">
          <div
            ...
            :class="cryptoStore.activePage === pages[index] ? 'bg-indigo-50 border-indigo-500 text-indigo-600' : ''"
    ...
    <div class="flex items-center justify-between md:hidden">
      <button class="flex items-center px-4 py-2.5 bg-white rounded-md" @click="previousPage()">
        <ChevronLeftIcon class="w-5 h-5" aria-hidden="true" />
        <span>Previous</span>
      </button>
